...
function Phidget(){
    ...
    function update(buffer){
        ...
        for( index in lines ){
            ...
            if(!chunk){
                ...
                if(lines[index].indexOf('200 inter-report')>-1){
                    this.emit(
                        'changed',
                        {
                            type:'Rate',
                            key:'rate',
                            value:lines[index].replace('200 inter-report period adjusted to ','')
                                .replace('ms','')
                        }
                    );
                    continue;
            ...
            */
            ...
            if(chunk[2]==('Attached')||chunk[2]==('Detached')){
                if(chunk[2]=='Detached'){
                    setTimeout(
                        function(){
                            this.ready=false;
    ...
    function set(params){
        ...
        if(!this.ready){
            this.emit(
                'error',
                'phidget is not attached, or not yet ready for set commands, try listening for the attached or phidgetReady commands.'
            )
            return;
        }
        ...
        switch(params.type){
            ...
            default :
                ...
                var paramNumber=Number(params.value);
                if(params.type=='Input'){
                    errorString='Inputs are for reading data only. You can not SET them.'
                }
                if(params.type=='Sensor'){
                    errorString='Sensors are for reading data only. You can not SET them.'
                }
                if(
                    params.type=='Output'
                    && (
                    ...
                    errorString='Outputs can only be set to 1 or 0'
                }
                if(errorString){
                    this.emit(
                        'error',
